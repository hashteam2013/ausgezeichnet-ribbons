...
if (isset($app['POST']['place_order'])) {
	...
	if( $acceptedAGB!='1')
	{
 		set_alert('error', 'Bitteasdf akzeptieren Sie die AGBs222222.');
    ...
    if (isset($app['POST']['shipping_check'])) {
        if (isset($app['user_key'])) {
            ...
            $paddr = isset($app['POST']['paddr']) ? $app['POST']['paddr'] : "";
            ...
            if ($fname == "") {
                ...
                $query->Data['billing_address2'] = $paddr;
                ...
                $query->Data['shipping_address2'] = $paddr;
           ...
           //         $to = $email;
            //        $subject = "Ribbons Order";
            //        $txt = 'Your ribbons order is placed successfully';
            //        $headers = 'FROM:'. FROM_EMAIL . "\r\n" . 'Reply-To:'.REPLY_TO_EMAIL . "\r\n" . 'X-Mailer: PHP/' . phpversion();
            //        mail($to,$subject,$txt,$headers);
                    if(SEND_NEW_ORDER_EMAIL_TO_ADMIN == '1'){
                   $to_admin = ADMIN_EMAIL;
                   $subject_admin = "Bestellaktivität";
                   $txt_admin = $email . ' '.  'hat eine Bestellung platziert, aber noch keine Zahlung getätigt.';
        ...
        if (isset($app['user_key'])) {
            ...
            $p_saddr = isset($app['POST']['ps_addr']) ? $app['POST']['ps_addr'] : "";
...
$query->Field = "ci.id,ci.customer_id,ci.quantity,ci.product_id,ci.total_price_tax_excl,ci.unit_price,ci.type,ci.number,ci.country,ci.year,b.batch_image,b.ribbon_name_".$app['language']." as ribbon_name_en,concat(c.first_name,' ',c.last_name) as customer_name";
