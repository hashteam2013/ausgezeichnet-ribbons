...
switch ($action):
        ...
        if ($useremail == "") {
            ...
            if (!empty($user_info)) {
                if ($user_info->is_active == '1' && $user_info->is_verified == '1') {
                    ...
                    $query1->Where = "where user_id = " . $user_info->id;
    ...
    case 'cust_detail':
	    ...
	    if ($cust_detail->ShownName=='')
		{
			...
			else if($cust_district->district==19)
			{
    ...
    case 'rename_cust':
        ...
        if (!empty($cust_id && $cust_fname && $cust_lname)) {
            $query = new query('customers');
    ...
    case 'rename_cust_ShownName':
        ...
        if (!empty($cust_id && $cust_ShownName)) {
            $query = new query('customers');
            $query->Data['id'] = $cust_id;
    ...
    case 'add_list':
		...
		$name_addition="";
            ...
            if (empty($record) || $app['POST']['batch_id'] == '548' || $app['POST']['batch_id'] == '549' || $app['POST']['batch_id'] == '551') {
                $query = new query('customer_batches');
                $query->Data['user_id'] = $app['POST']['user_id'];
                $query->Data['customer_id'] = $app['POST']['cust_id'];
                $query->Data['batch_id'] = $app['POST']['batch_id'];
                $query->Data['type'] = $app['POST']['batchType'];
                $query->Data['date_add'] = '1';
                if ($query->Insert()) {
                    $show_batch = array();
...
//                    $batch_details = $query1->DisplayOne();
                ...
                if (is_object($check_cart_entry) && $cust->id != '548' && $cust->id != '549' && $cust->id != '551') {
                        $output['msgs'][] = array(
	...
	else if(is_object($check_cart_entry) && ($cust->id == '548' || $cust->id == '549' || $cust->id == '551'))
	{
