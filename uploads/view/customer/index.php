                        ...
                        if (!empty($customers)) {
                            foreach ($customers as $cust) {
                                ...
                                <tr>
                                    ...
                                    <td><input type="button" name="rename_cust"  data-toggle="modal" data-target="#renamepopup" onclick="RenameFunction(<?php echo $cust['id']; ?>)" data-cid ="<?php echo $cust['id']; ?>" class="btn btn-info btn-sm" value="<?php _e("Rename Customer"); ?>"></td>
...
<div id="renamepopup" class="modal fade in" role="dialog" style="display: none; padding-right: 15px;">
    <div class="modal-dialog">
                        ...
                        <form role="form" action="" id="customer_add" method="POST">
                            ...
                            <div class="form-body">
                                ...
                                <div class="col-sm-6 col-sm-offset-3">
                                    <button type="submit" name="cust-submit"  class="form-control hvr-float-shadow " tabindex="4" id="cust-rename"  tabindex="4" ><?php _e("Rename Customer"); ?></button>
                                </div>
                            </div>
                        </form>
                    </div>
...
<script>
    ...
    function RenameFunction(id) {
        var cid = $('#renamepopup #data_cid').val(id);
        $.ajax({
            type: 'POST',
    ...
    jQuery(document.body).on('click', "#cust-rename", function (e) {
        e.preventDefault();
        var id = $('#renamepopup #data_cid').val();
        var fname = $("#fname").val();
        var lname = $("#lname").val();
        $.ajax({
            type: 'POST',
