            ...
            <div class="categories">
                ...
                <ul>
                    <?php foreach ($categories as $key=>$cat) { ?>
                        <li><label><input type="checkbox" name="categories_name[]" class="cat_class" id="cat_id_<?php echo $cat->position ?>"  dist-attr="<?php echo $cat->is_district_related; ?>" value="<?php echo $cat->id ?>"><?php echo $cat->{'name_'.$app['language']}; ?></label></li>
                    <?php } ?>
                </ul>
            </div>
                            ...
                            <ul class="batch">
                                ...
                                <li id="ribbon_<?php echo $batch['id']; ?>">
                                   <?php echo show_ribbon_images($batch['type'],$batch['batch_image'],$batch['number'],$batch['country'],$batch['batch_id']); ?>
                                    <span><label><input type="checkbox" class="chkid" value="<?php echo $batch['id']; ?>"><?php echo $batch['ribbon_name_'.$app['language']] . '  |  ' . $batch['level'] . '<br>' . $batch['grade'] . ' / ' . $batch['value']; ?></label></span>
                                </li>
                                <?php } ?>
                            </ul>
                        </div>
...
<script>
 ...
 $.ajax({
	...
	nails=4;
                ...
                $('.badges').html('<div class=srch-reslt slect mar-top-10><div class=srch-heading><?php _e("Badges Placed"); ?></div><div class=flag-contaner>' + '<p> Shop.php: F&uumlr diese Spange ben&oumltigen Sie ' + LConnectors +  " L&aumlngsverbinder und " + QConnectors + " Querverbinder. Wir empfehlen " + nails + " N&aumlgel. Bitte passen Sie die St&uumlckzahlen im Warenkorb an. </p>" + badgeData + '</div></div></div>');
            }
        });
