...
$query->Where = " where is_active = '1' order by position";
$get_ia_lev1 = $query->ListOfAllRecords();
switch ($action):
case 'add':
 if (isset($app['POST']['add'])) {
            ...
            if($app['POST']['name_ia'] == ''){
               $msg = 'Please choose international authority first'; 
                ...
                $queryObj = new query('international_authorities_sublevel2');
                $queryObj->Field = " id";
                ...
                if(!is_object($object)){
                    ...
                    if ($query->Insert()) {
                        ...
                        redirect(app_url('international-authorities-sublevel2','list','list',array(),true));
                    } else {
                        $msg = 'Error occurred while updating account info. Please try again!';
                    }
                     } else{
                    $msg = 'international authority sublevel2s name already exist';   
                }
            }
            set_alert('error', $msg);
        }
break;
    ...
    case 'edit':
            ...
            if (trim($app['POST']['nameen']) == '') {
                ...
                $msg = 'Please enter position';
                    ...
                    if ($query->Update()) {
                        ...
                        redirect(app_url('international-authorities-sublevel2','edit','edit',array('id'=>$id),true));
                    } else {
                        $msg = 'Error occurred while updating account info. Please try again!';
                    }
            }
        ...
        $query = new query('international_authorities_sublevel2 as ia_auth');
        $query->Field = "ia_auth.id,ia_auth.ia_id as ia_id,ia_auth.ia_lev1_id as ia_lev1_id,ia_auth.name_en,ia_auth.name_dr,ia_auth.position,ia_auth.is_active,ia.name_en as ia_name_en,ia_sublev1.name_en as sublev2_name";
        $query->Where = " LEFT JOIN international_authorities as ia ON ia.id = ia_auth.ia_id";
        $query->Where .= " LEFT JOIN international_authorities_sublevel1 as ia_sublev1 ON ia_sublev1.id = ia_auth.ia_lev1_id";
        $query->Where .= " where ia_auth.id = ".$id;
        //$query->print=1;
        $ia_sub1 = $query->DisplayOne();
       //pr($ia_sub1);
        if(!(is_object($ia_sub1))){
            redirect(app_url('international-authorities-sublevel2','list','list',array('id'=>$id),true));
        }
        break;
...
case 'list':
        ...
        $sublev2ia = $query->ListOfAllRecords('object');
        break;
endswitch;
?>
