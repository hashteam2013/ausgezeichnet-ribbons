...
// -------- Invoice Text-------
    ...
    $pdf->SetFont('','B');
    $pdf->Cell(0,10,'Stornorechnung SR-W-1'.str_pad ($orders->id,5,'0',STR_PAD_LEFT),0,1);

    $pdf->SetFont('','');
  $pdf->Cell(0,10, 'Sehr geehrte Damen und Herren, hiermit stellen wir Ihnen folgende Gutschrift aus:',0,1);
     $pdf->Cell(0,10, 'Grund: Storno der Bestellung zu Rechnung AR-W-1'.str_pad ($orders->id,5,'0',STR_PAD_LEFT),0,0);
...
if ($orders->discount==1)
{
        ...
        if ($orders->is_payment_made == 1) 
        {
            ...
            {
            $pdf->Cell(0,8,utf8_decode('Rechnung AR-W-1'.str_pad ($orders->id,5,'0',STR_PAD_LEFT) . ' am '.date("d.m.Y", $time_add) .' per PayPal vollständig bezahlt.'),0,1);
            }
            elseif($orders->payment_method_name=="Sofort")
            {
            $pdf->Cell(0,8,utf8_decode('Rechnung AR-W-1'.str_pad ($orders->id,5,'0',STR_PAD_LEFT) . ' am '.date("d.m.Y", $time_add) .' per Sofort Überweisung vollständig bezahlt.'),0,1);                          
            }
            elseif($orders->payment_method_name=="banktransfer")
            {
             $pdf->Cell(0,8,utf8_decode('Rechnung AR-W-1'.str_pad ($orders->id,5,'0',STR_PAD_LEFT) . ' am '.date("d.m.Y",$time_payed) .' per Überweisung vollständig bezahlt.'),0,1);                           
            }
        }
        ...
        if ($orders->is_payment_made == 0) 
        {
             ...
             $pdf->Cell(0,8,utf8_decode('Betrag wird mit der offenen Rechnung AR-W-1'.str_pad ($orders->id,5,'0',STR_PAD_LEFT).' ausgeglichen. Es erfolgt daher keine Geldüberweisung.'),0,1);
        }
else
{
           
             $pdf->Cell(0,8,utf8_decode('Der Betrag wurde am ' . date("d.m.Y",$time_today) . ' rücküberwiesen.'),0,1);
    ...
    $pdf->Output('', 'Rechnung SR-W-1'.str_pad ($orders->id,5,'0',STR_PAD_LEFT).'.pdf');
?>
