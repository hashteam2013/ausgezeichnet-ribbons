...
// -------- Invoice Text-------
    ...
    $pdf->SetFont('','B');
    $pdf->Cell(0,10,'Letzte Mahnung zu Rechnung AR-W-1'.str_pad ($orders->id,5,'0',STR_PAD_LEFT),0,1);
//$pdf->Cell(0,10,'Angebot A-W-1'.str_pad ($orders->id,5,'0',STR_PAD_LEFT),0,1);
	...
	if($orders->override_order_date!="")
	{
    		...
    		$pdf->Write(6, 'Sehr geehrte Damen und Herren, leider ist trotz mehrfacher Erinnerungen zu Ihrer Bestellung vom ' . $orders->override_order_date . ' noch keine Zahlung eingegangen.');

	}
	else
	{
    		$pdf->Write(6, 'Sehr geehrte Damen und Herren, leider ist trotz mehrfacher Erinnerungen zu Ihrer Bestellung vom '.date("d.m.Y",$time).'  noch keine Zahlung eingegangen.');
	}
...
UTFencoder2($order_item->batch_name).$text_Str.$empf_Str,str_pad (number_format($order_item->unit_price,2,",",".") ,12,' ',STR_PAD_LEFT),str_pad (number_format($order_item->quantity,2,",","."),8,' ',STR_PAD_LEFT),str_pad (number_format($order_item->unit_price * $order_item->quantity,2,",","."),12,' ',STR_PAD_LEFT)));
  	...
  	$pdf->Write(6, UTFencoder2('Wir ersuchen Sie letztmalig um umgehende Bezahlung der ausstehenden Summe. Sollten wir binnen 10 Tagen keinen Zahlungseingang verbuchen, sehen wir uns gezwungen Mahnklage einzureichen, wobei wir uns hinsichtlich anfallender Gebühren bei Ihnen schadlos halten werden.'));
    ...
    $pdf->Output('F', '../ad-min/pdfAblage/'.$orders->id.'/'.'Mahnung zu AR-W-1'.str_pad ($orders->id,5,'0',STR_PAD_LEFT).'('.date("d.m.Y").').pdf');
