...
switch ($action):
    case'view': 
        ...
        $query_one -> Field = "oi.*, b.batch_image as batch_image,  b.ItemType as ItemType, b.ribbon_name_en as batch_name,b.type as type, b.is_batch as is_batch2, c.first_name as firstname,c.last_name as lastname, c.ShownName as ShownName, descr.Text as text";
        ...
        $query_one->Where.= " where oi.inactive=0 AND oi.order_id =$id ORDER BY oi.pos,oi.customer_id, b.level, b.value, oi.id";
...
UTFencoder2($order_item->batch_name).$text_Str.$empf_Str,str_pad (number_format($order_item->unit_price/1.2,2,",",".") ,12,' ',STR_PAD_LEFT),str_pad (number_format($order_item->quantity,2,",","."),8,' ',STR_PAD_LEFT),str_pad (number_format($order_item->unit_price * $order_item->quantity/1.2,2,",","."),12,' ',STR_PAD_LEFT)));
...
//        $pdf->Cell($w[4],6,str_pad (number_format(($orders->grand_total*(1-$Discount)+$ShippingCosts)*0.166667,2,",","."),12,' ',STR_PAD_LEFT),'TBR',1,'L');
        ...
        $pdf->Cell(0,3,utf8_decode(''),0,1);
                $pdf->Cell(0,3,UTFencoder2(''),0,2);
                $pdf->Cell(0,3,UTFencoder2('Preis: Nettopreis zuzüglich 20% Umsatzsteuer, Lieferung frei Haus'),0,1);
       ...
       if ($orders->is_payment_made == 0) 
        {
            ...
            {
                $pdf->Cell(0,4,UTFencoder2('Zahlungsmethode: Rechnung; Zahlungsziel ' . $paymentcondition->name_dr),0,1); 
