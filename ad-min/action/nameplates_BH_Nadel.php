...
$limit = PAGE_CONTENT_LIMIT;
        ...
        $query_one->Where.= " where oi.product_id='1216' AND o.is_packing_made=0 AND o.is_order_valid=1 ORDER BY c.last_name, c.first_name";
...
$actual_lastname = "";
	...
	{ 
		...
		{
    		$pdf->CellFit(63.1,6.8,UTFencoder2(($order_item->ShownName)),0,1,'C','','','true');
		}
		else
		{
  		$pdf->Cell(63.1,6.8,UTFencoder2(($order_item->ShownName)),0,1,'C','','');
		}
	}
         }

    $pdf->Output('', 'Offene Namensschilder BH Nadel '.date('Y-m-d').'.pdf');
?>
