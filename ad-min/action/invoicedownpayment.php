...
// -------- Invoice Text-------
    ...
    $pdf->Cell(0,10,'Anzahlungsrechnung AR-W-1'.str_pad ($orders->id,5,'0',STR_PAD_LEFT),0,1);
...
if ($orders->discount==1)
{
        ...
        $pdf->SetFont('','');
        $pdf->Cell($w[0],6,'','LTB',0);
        $pdf->Cell($w[1],6,'davon USt','TB',0);
        $pdf->Cell($w[2],6,'','TB',0);
        $pdf->Cell($w[3],6,'EUR','TB',0);
        $pdf->Cell($w[4],6,str_pad (number_format(($orders->grand_total*(1-$Discount)+$ShippingCosts)*0.2/1.2,2,",","."),12,' ',STR_PAD_LEFT),'TBR',1,'L');
       ...
       $pdf->SetFont('','B');
        $pdf->Cell($w[0],6,'','LTB',0);
        $pdf->Cell($w[1],6,'Anzahlung ' . $orders->downpayment . '%' ,'TB',0);
        $pdf->Cell($w[2],6,'','TB',0);
        $pdf->Cell($w[3],6,'EUR','TB',0);
        $pdf->Cell($w[4],6,str_pad (number_format($orders->grand_total*$orders->downpayment/100,2,",","."),12,' ',STR_PAD_LEFT),'TBR',1,'L');
       ...
       $pdf->SetFont('','');
        ...
        $pdf->Cell($w[2],6,'','TB',0);
        $pdf->Cell($w[3],6,'EUR','TB',0);
        $pdf->Cell($w[4],6,str_pad (number_format($orders->grand_total*$orders->downpayment/100/1.2*0.2,2,",","."),12,' ',STR_PAD_LEFT),'TBR',1,'L');
        ...
        if ($orders->is_payment_made == 0) 
        {
            ...
            $pdf->Cell(0,8,UTFencoder2('Bitte überweisen Sie den gesamten Betrag von EUR '.number_format($orders->grand_total*$orders->downpayment/100,2,",",".") . ' auf untenstehendes Konto.'),0,1);
        }
