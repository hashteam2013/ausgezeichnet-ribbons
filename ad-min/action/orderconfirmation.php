...
switch ($action):
    case'view': 
        ...
        $query_one->Where.= " where oi.order_id =$id ORDER BY oi.pos, oi.customer_id, b.level, b.value";
...
// -------- Invoice Text-------
    ...
    $pdf->SetFont('','B');
    		...
    		$pdf->Cell(0,10,UTFencoder2('Auftragsbestätigung AB-W-1').str_pad ($orders->id,5,'0',STR_PAD_LEFT),0,1);
	...
	if($orders->override_order_date!="")
	{
    		...
    		$pdf->Write(6, UTFencoder2('Sehr geehrte Damen und Herren, entsprechend Ihrer Bestellung vom ') . $orders->override_order_date . UTFencoder2(' möchten wir Ihnen folgenden Auftrag bestätigen:'));

	}
	else
	{
    		$pdf->Write(6, UTFencoder2('Sehr geehrte Damen und Herren, entsprechend Ihrer Bestellung vom ').date("d.m.Y",$time).UTFencoder2(' möchten wir Ihnen folgenden Auftrag bestätigen:'));
	}
...
if ($orders->discount==1)
{
 ...
 //  //             $pdf->Cell(0,2,UTFencoder2('Zahlungsmethode: Rechnung; Zahlungsziel 14 Tage ohne Abzug.'),0,1);
            ...
            $pdf->Cell(0,8,UTFencoder2('Bitte überweisen Sie den gesamten Betrag von EUR '.number_format($orders->grand_total*(1-$Discount)+$ShippingCosts,2,",",".") .' auf untenstehendes Konto.'),0,1);
        }
...
if (get_discount_string_admin($orders->billing_zip)<>"")
{
	...
	else
	{
		...
		{
	   	     $pdf->Cell(0,8,"Versanddatum: 3 Werktage ab Bestelldatum",0,1);
		}
		else
		{
