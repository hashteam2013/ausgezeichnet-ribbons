...
$queryList[] = $records[0];
$departmentIDList[]=$app['user_info']->department_new;    
...
if($records_size>1)
{
    ...
    for ($i=1;$i<=$records_size-1;$i++)
    {
        ...
        if (($records[$i]['depart_id']!=$records[$i-1]['depart_id']) || ($records[$i]['dist_id']!=$records[$i-1]['dist_id']) || ($records[$i]['subdist_id']!=$records[$i-1]['subdist_id']))
        {
            ...
            if($records[$i]['depart_id']!=$departmentIDList[count($departmentIDList)-1] && $records[$i]['depart_id']!=$app['user_info']->department_new)
            {
