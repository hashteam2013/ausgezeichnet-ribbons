...
//    $pdf->Write(6, 'Sehr geehrte Damen und Herren, entsprechend Ihrer Bestellung vom 13.11.2017 stellen wir folgende Leistungen in Rechnung:');
//$pdf->Write(6, UTFencoder2('Sehr geehrte Damen und Herren, danke für Ihr Interesse an unseren Produkten. Hiermit bieten wir folgende Leistungen an:'));
    $pdf->Ln(10);
...
UTFencoder2($order_item->batch_name).$text_Str.$empf_Str,str_pad (number_format($order_item->unit_price*0.833333,2,",",".") ,12,' ',STR_PAD_LEFT),str_pad (number_format($order_item->quantity,2,",","."),8,' ',STR_PAD_LEFT),str_pad (number_format($order_item->unit_price*0.833333 * $order_item->quantity,2,",","."),12,' ',STR_PAD_LEFT)));
        ...
        $pdf->Cell($w[1],6,'Summe','TB',0);
        $pdf->Cell($w[2],6,'','TB',0);
        $pdf->Cell($w[3],6,'EUR','TB',0);
        $pdf->Cell($w[4],6,str_pad (number_format($orders->grand_total*0.8333333,2,",","."),12,' ',STR_PAD_LEFT),'TBR',1,'L');
...
if ($orders->discount==0.2)
{
        ...
        $pdf->Cell($w[2],6,'','TB',0);
        $pdf->Cell($w[3],6,'EUR','TB',0);
        $pdf->Cell($w[4],6,str_pad (number_format($orders->grand_total*0.8333333*(1-$Discount),2,",","."),12,' ',STR_PAD_LEFT),'TBR',1,'L');
}
...
if ($orders->discount==1)
{
        ...
        $pdf->Cell($w[2],6,'','TB',0);
        $pdf->Cell($w[3],6,'EUR','TB',0);
        $pdf->Cell($w[4],6,str_pad (number_format($ShippingCosts*0.83333,2,",","."),12,' ',STR_PAD_LEFT),'TBR',1,'L');
        $pdf->SetFont('','B');
        $pdf->Cell($w[0],8,'','LTB',0);
        $pdf->Cell($w[1],8,'Gesamtsumme mit Versand','TB',0);
        $pdf->Cell($w[2],8,'','TB',0);
        $pdf->Cell($w[3],8,'EUR','TB',0);
        $pdf->Cell($w[4],8,str_pad (number_format(($orders->grand_total*(1-$Discount)+$ShippingCosts)*0.83333333,2,",","."),12,' ',STR_PAD_LEFT),'TBR',1,'L');
...
$pdf->SetFont('','',10);
        $shipping_string =SHIPPING_TIME_TITLE . ": " . SHIPPING_TIME_VALUE;
            ...
            $pdf->Cell(0,8,UTFencoder2('Innergemeinschaftliche Lieferung, steuerfrei gem. Art. 6 Abs. 1 UStG'),0,1);
        ...
        if ($orders->is_payment_made == 0) 
        {
            ...
            $pdf->Cell(0,8,UTFencoder2('Bitte überweisen Sie den gesamten Betrag von EUR '.number_format($orders->grand_total*0.83333*(1-$Discount)+$ShippingCosts,2,",",".") .' auf untenstehendes Konto.'),0,1);
        }
