...
UTFencoder2($order_item->batch_name).$text_Str.$empf_Str,str_pad (number_format($order_item->unit_price/1.2,2,",",".") ,12,' ',STR_PAD_LEFT),str_pad (number_format($order_item->quantity,2,",","."),8,' ',STR_PAD_LEFT),str_pad (number_format($order_item->unit_price/1.2 * $order_item->quantity,2,",","."),12,' ',STR_PAD_LEFT)));
        ...
        $pdf->Cell($w[1],6,'Summe exkl. USt','TB',0);
        $pdf->Cell($w[2],6,'','TB',0);
        $pdf->Cell($w[3],6,'EUR','TB',0);
        $pdf->Cell($w[4],6,str_pad (number_format($orders->grand_total/1.2,2,",","."),12,' ',STR_PAD_LEFT),'TBR',1,'L');
...
if ($orders->discount==0.2)
{
        ...
        $pdf->Cell($w[2],6,'','TB',0);
        $pdf->Cell($w[3],6,'EUR','TB',0);
        $pdf->Cell($w[4],6,str_pad (number_format($orders->grand_total/1.2*(1-$Discount),2,",","."),12,' ',STR_PAD_LEFT),'TBR',1,'L');
}
...
if ($orders->discount==0.07)
{
        ...
        $pdf->Cell($w[2],6,'','TB',0);
        $pdf->Cell($w[3],6,'EUR','TB',0);
        $pdf->Cell($w[4],6,str_pad (number_format($ShippingCosts/1.2,2,",","."),12,' ',STR_PAD_LEFT),'TBR',1,'L');
        $pdf->SetFont('','');
        $pdf->Cell($w[0],8,'','LTB',0);
        $pdf->Cell($w[1],8,'Gesamtsumme exkl. USt','TB',0);
        $pdf->Cell($w[2],8,'','TB',0);
        $pdf->Cell($w[3],8,'EUR','TB',0);
        $pdf->Cell($w[4],8,str_pad (number_format(($orders->grand_total*(1-$Discount)+$ShippingCosts)/1.2,2,",","."),12,' ',STR_PAD_LEFT),'TBR',1,'L');
        $pdf->Cell($w[0],6,'','LTB',0);
        $pdf->Cell($w[1],6,'USt (20%)','TB',0);
        $pdf->Cell($w[2],6,'','TB',0);
        $pdf->Cell($w[3],6,'EUR','TB',0);
        $pdf->Cell($w[4],6,str_pad (number_format(($orders->grand_total*(1-$Discount)+$ShippingCosts)/1.2*0.2,2,",","."),12,' ',STR_PAD_LEFT),'TBR',1,'L');
        $pdf->SetFont('','B');
        $pdf->Cell($w[0],6,'','LTB',0);
        $pdf->Cell($w[1],6,'Gesamtsumme inkl. USt','TB',0);
        $pdf->Cell($w[2],6,'','TB',0);
        $pdf->Cell($w[3],6,'EUR','TB',0);
        $pdf->Cell($w[4],6,str_pad (number_format(($orders->grand_total*(1-$Discount)+$ShippingCosts),2,",","."),12,' ',STR_PAD_LEFT),'TBR',1,'L');
        $pdf->SetFont('','');
       ...
       if ($orders->is_payment_made == 0) 
        ...
        $pdf->Cell(0,8,utf8_decode('Dieses Angebot ist 30 Tage gültig.'),0,1);
        $pdf->Cell(0,4,utf8_decode(''),0,1);
        $pdf->Cell(0,8,utf8_decode('Wir würden uns sehr freuen, wenn Ihnen unser Angebot zusagt.'),0,1);
