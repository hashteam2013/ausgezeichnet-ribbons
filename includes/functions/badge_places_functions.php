...
function listBatch($show) {
    ...
    $batch_id->Where = " where id = '" . $app['user_info']->id . "'";
    ...
    if (!empty($DdeaprtID->name_en)) {
        ...
        $user_serious_level=$DdeaprtID->serious_level;
	if($app['user_info']->forcewearall=='1'){
      	 $user_serious_level=0;
	}
    } else {
        $userdepart_en = '';
       $user_serious_level=0;
    ...
    $query_1->Where .= " where cust.customer_id = '" . $show . "' and batch.level ='1' and batch.serious_level>=$user_serious_level order by batch.value asc";
    //$query_1->print=1;
    $showing_b_id_1 = $query_1->ListOfAllRecords();
    //echo "<pre>";print_r($showing_b_id_1);

    /*     * ****** Level 2 for batchtype = 0  ******** */
    $showing_b_id_2 = levelTypeTwo($show, $districtID, $userid, $username_en, $user_serious_level );

    /**     * *****Get all batch from level 3 ******** */
    $showing_b_id_3 = levelTypeThree($show, $username_en, $usercomm_en, $user_serious_level );

    /*     * ******Level 4 AND 5******** */

    $showing_b_id_4 = levelTypeFour($show, $districtID, $userid, $username_en, $userdepart_en, $usersubdist_en, $user_serious_level );
    $showing_b_id_5 = levelTypeFive($show, $districtID, $userid, $username_en, $user_serious_level );
...
//    }
    ...
    if (!empty($showing_b_id)) {
        ...
        $query_pre->Where .= " where usr.id = '" . $app['user_info']->id . "'";
        //$query_pre->print=1;
        $presort = $query_pre->DisplayOne();
        $match_arr = $recordd = $common_arr = array();
	
        $totalBeforePresorting=sizeof($output);

        if (($presort->depart_class == '1' || $presort->dist_class == '1') &&  $app['user_info']->forcewearall=='0') {
            //echo "hii";exit();
       ...
       $presortedElements[] = $totalBeforePresorting;
    ...
    $query_2->Field = " batch.id as batch_id,batch.value,"
    ...
    $query_2->Where .= " LEFT JOIN batch_filter_relation as filter1 ON batch.id = filter1.batch_id and filter1.rel_type = 'departments_new' ";
    $query_2->Where .= " LEFT JOIN batch_filter_relation as dist ON dist.batch_id = cust.batch_id and dist.rel_type='districts' ";
    $query_2->Where .= " LEFT JOIN batch_filter_relation as sbdist ON sbdist.batch_id = cust.batch_id and sbdist.rel_type='subdistricts' ";
    $query_2->Where .= " LEFT JOIN sub_districts as sbds ON sbds.id = sbdist.filter_id ";
    $query_2->Where .= " LEFT JOIN batch_filter_relation as bfr1 ON bfr1.batch_id = cust.batch_id and bfr1.rel_type='communities' ";
    $query_2->Where .= " LEFT JOIN communities as comm ON comm.id = bfr1.filter_id ";
    $query_2->Where .= " LEFT JOIN districts as ds ON ds.id = dist.filter_id ";
    $query_2->Where .= " LEFT JOIN departments_new as depart ON depart.id = filter1.filter_id ";
    $query_2->Where .= " where batch.level < '6'  and cust.customer_id = '" . $show . "'  order by batch.value, distname";

    $records = $query_2->ListOfAllRecords();
        $totalBeforePresortingIntegrity=sizeof($records);


      $presortedElements[] = $totalBeforePresortingIntegrity;
...
/**   Level 2 for batchtype = 0  * */
function levelTypeTwo($show, $districtID, $userid, $username_en, $user_serious_level ) {
    global $app;
    ...
    $query_2->Where .= " LEFT JOIN districts as ds ON ds.id = dist.filter_id ";
    $query_2->Where .= " LEFT JOIN departments_new as depart ON depart.id = filter1.filter_id ";
    $query_2->Where .= " where cust.customer_id = '" . $show . "' and batch.level ='2' and batch.serious_level>=$user_serious_level order by batch.value, distname";
...
if(empty($records))
{
 ...
 //    get minimum values from array if having same district
$queryList= array();
...
$queryList[] = $records[0]['dist_id'];


if($records_size>1)
{

//Check if user district is in the record list. If so, put this item first.

    $queryList[0]= $records[0]['dist_id'];
    for ($i=0;$i<=$records_size-1;$i++)
    {
        if($records[$i]['dist_id']==$app['user_info']->district)
        {
            $queryList[0]=$app['user_info']->district; 
            break;
        }
    }  
  
    
//Now, group by district
    for ($i=0;$i<=$records_size-1;$i++)
    {
	$donotadd=false;
        foreach ($queryList as $queryItem)
        {
	if($records[$i]['dist_id']== $queryItem)
        	{
            		$donotadd=true;
        	}
       }
    ...
    if (!empty($queryList) && !empty($records))
    {
        foreach ($queryList as $queryItem)
        {
            foreach ($records as $recordItem) {
                if ($recordItem['dist_id'] ==$queryItem)
                {
...
function levelTypeThree($show, $username_en, $usercomm_en, $user_serious_level ) {
    global $app;
    $query_3 = new query("batches as batch");
    $query_3->Field = " batch.id as batch_id,batch.value,"
            . "batch.grade,batch.ribbon_name_" . $app['language'] . " as ribbon_name_en,batch.level,batch.type,batch.batch_image,cust.number,depart.position as depart_pos,cust.country,cust.id as id,cust.year,depart.id as depart_id,depart.name_" . $app['language'] . " as department, ds.id as dist_id,ds.name_" . $app['language'] . " as distname ,sbds.id as subdist_id,sbds.name_" . $app['language'] . " as subdist,comm.id as comm_id,comm.name_" . $app['language'] . " as comm_name";
    $query_3->Where = " LEFT JOIN customer_batches as cust ON batch.id = cust.batch_id";
    $query_3->Where .= " LEFT JOIN batch_filter_relation as filter1 ON batch.id = filter1.batch_id and filter1.rel_type = 'departments_new' ";
    $query_3->Where .= " LEFT JOIN batch_filter_relation as dist ON dist.batch_id = cust.batch_id and dist.rel_type='districts' ";
    $query_3->Where .= " LEFT JOIN batch_filter_relation as sbdist ON sbdist.batch_id = cust.batch_id and sbdist.rel_type='subdistricts' ";
    $query_3->Where .= " LEFT JOIN sub_districts as sbds ON sbds.id = sbdist.filter_id ";
    $query_3->Where .= " LEFT JOIN batch_filter_relation as bfr1 ON bfr1.batch_id = cust.batch_id and bfr1.rel_type='communities' ";
    $query_3->Where .= " LEFT JOIN communities as comm ON comm.id = bfr1.filter_id ";
    $query_3->Where .= " LEFT JOIN districts as ds ON ds.id = dist.filter_id ";
    $query_3->Where .= " LEFT JOIN departments_new as depart ON depart.id = filter1.filter_id ";
    $query_3->Where .= " where cust.customer_id = '" . $show . "' and batch.level ='3'  and batch.serious_level>=$user_serious_level order by batch.value, distname , comm_name";

    $records = $query_3->ListOfAllRecords();
...
//Check if user district is in the record list. If so, put this item first.
    ...
    $queryList[0]= $records[0];
    for ($i=0;$i<=$records_size-1;$i++)
    {
        if($records[$i]['dist_id']==$app['user_info']->district)
        {
            $queryList[0]=$records[$i];
            break;
        }
    }  
...
//Now, group by district
    for ($i=0;$i<=$records_size-1;$i++)
    {
        ...
        foreach ($queryList as $queryItem)
        {
	if($records[$i]['dist_id']== $queryItem['dist_id'])
        	{
            		$donotadd=true;
        	}
       }
    ...
    // Start Loop through that list containing only one community per district. Query every element in that list and get all items with that district.
//Result: All items, grouped by District, districts sorted by value
    ...
    if (!empty($queryList) && !empty($records))
    {
        ...
        {
            foreach ($records as $recordItem) {
                if ($recordItem['dist_id'] ==$queryItem['dist_id'])
                {
                    $merge [] = $recordItem;
                }
            }
...
$records_size2=count($merge);
  
//Check if user community is in the record list. If so, put this item first.

    for ($i=0;$i<=$records_size2-1;$i++)
    {
        if($merge[$i]['comm_id']==$app['user_info']->community)
        {
            $queryList[0]=$merge[$i];
            break;
        }
...
//Now, group by community
    for ($i=0;$i<=$records_size2-1;$i++)
    {
        $donotadd=false;
        foreach ($queryList as $queryItem)
        {
	if($merge[$i]['comm_id']== $queryItem['comm_id'])
        	{
            		$donotadd=true;
        	}
       }
    ...
    // Start Loop through that list. Query every element in that list from List#1 by community and District and insert the results.
    ...
    if (!empty($queryList) && !empty($records))
    {
        ...
        {
            foreach ($records as $recordItem) {
                if ($recordItem['dist_id'] ==$queryItem['dist_id'] && $recordItem['comm_id'] ==$queryItem['comm_id'])
                {
                    $merge2_1final[] = $recordItem;
                }
            }
...
function levelTypeFour($show, $districtID, $userid, $username_en, $userdepart_en, $usersubdist_en, $user_serious_level ) {
    global $app;
    ...
    $query_4->Where .= " where cust.customer_id = '" . $show . "' and batch.level ='4' and batch.serious_level>=$user_serious_level order by depart_pos, comm_name, subdist,  distname, batch.value";
...
if(empty($records))
{
	...
	$userdep= $app['user_info']->department_new;
    ...
    $query_d = new query("departments_possitions as dplist");
    $query_d->Field = " related_department, position ";
    $query_d->Where .= " where main_department = '". $userdep . "' order by position";
    $records_d = $query_d->ListOfAllRecords();
...
$records_size=count($records);
$records_size_d=count($records_d);
...
$departmentIDList[]= $userdep;

if($app['user_info']->department_new2!=0)
{
	$departmentIDList[]= $app['user_info']->department_new2;
}

    for ($i=0;$i<=$records_size_d-1;$i++)
    {
	if($app['user_info']->department_new2!=$records_d[$i]['related_department'])
	{
		$departmentIDList[]=$records_d[$i]['related_department'];
	}
    ...
    usort($queryList2,"compare_ribbon_departmentAndValue");
...
$queryList=array();
    ...
    $queryListSubdistrict= array();
        ...
        foreach($queryList2 as $queryItem)
        {
            if($queryItem['depart_id']==$depID)
            {
                ...
                {
                    if($queryItem['dist_id']==$app['user_info']->district)
                    {
        ...
        if (!empty($queryListSubdistrict))
        {
            ...
            if ($app['user_info']->district!=0)
            {
                $districtList[]=$app['user_info']->district;
            }
            else
            {
            ...
            foreach($districtList as $districtID)
            {
                foreach($queryListSubdistrict as $subdistrictItem)
                {   
                    if($districtID==$subdistrictItem['dist_id'] && $app['user_info']->subdistrict!=$subdistrictItem['subdist_id'])
                    {
                        $queryListSubdistrict2[]=$subdistrictItem;
                    }
                    elseif($districtID==$subdistrictItem['dist_id'] && $app['user_info']->subdistrict==$subdistrictItem['subdist_id'])
                    {
  ...
  function compare_ribbon_departmentAndValue($ribbon1, $ribbon2)
  {
...
function compare_ribbon_Value($ribbon1, $ribbon2)
  {
...
function levelTypeFive($show, $districtID, $userid, $DdistrictID, $user_serious_level) {
    ...
    $query_5->Where .= " where cust.customer_id = '" . $show . "'  and org.rel_type='ia' and batch.level = '5' and batch.serious_level>=$user_serious_level order by  Inter.position, Inter2.position, batch.value";
    //$query_5->print=1;
...
//     echo "<pre>";print_r($finallArr1);
    ...
    $query_5_2->Where .= " where cust.customer_id = '" . $show . "'  and org.rel_type='ia' and batch.level = '5' and batch.serious_level>=$user_serious_level order by batch.value";
...
function listBatchAll($show) {
    ...
    /*     * ****** Level 2 for batchtype = 0  ******** */
    $showing_b_id_2 = levelTypeTwo($show, $districtID, $userid, $username_en);

    /**     * *****Get all batch from level 3 ******** */
    $showing_b_id_3 = levelTypeThree($show, $username_en, $usercomm_en);

    /*     * ******Level 4 AND 5******** */

    $showing_b_id_4 = levelTypeFour($show, $districtID, $userid, $username_en, $userdepart_en, $usersubdist_en);
    $showing_b_id_5 = levelTypeFive($show, $districtID, $userid, $username_en);
