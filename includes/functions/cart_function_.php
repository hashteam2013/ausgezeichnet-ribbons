...
function get_discount_string()
{
	...
	if(substr($app['user_info']->zipcode,0,1)=='6' || substr($app['user_info']->zipcode,0,2)=='99')
	{
		 return UTF8_encode("");
	}
	return "";
	//return "Jubil&auml;umsaktion -7% ";
}
...
function place_order() {
    ...
    if (!is_object($record)) {
        ...
        $query->Data['total_shipping_amount'] = ShipingCost($grand_total*(1-$cart_details->discount));
	...
	if($ship_email->accepted_eMail==1)
	{
	            ...
	            $txt = 'Sehr geehrte/r ' . $cart_details->billing_firstname . ' ' . $cart_details->billing_lastname . '. Vielen Dank fuer Ihr Interesse an unseren Produkten. Wir haben Ihre Bestellung erhalten, und werden diese umgehend bearbeiten. Ihr Ausgezeichnet.cc - Team.';
